...
mr_results_neuroticism <- mr_results %>% filter(method %in% c("Inverse variance weighted")
                                                & outcome == "neuroticism") %>%
...
mr_results$colour <- rep(c("gray95","white"), 12)
...
p <- ggplot(mr_results, aes(x = odds_ratio, y = outcome, xmin = lower_CI, xmax = upper_CI, fill = outcome, shape = method)) +
  ...
  geom_pointrange(shape = c(rep(15,24)), fill = "black") +
...
mr_results_neuroticism$est <- paste0(sprintf("%.2f",mr_results_neuroticism$b)," (",sprintf("%.3f",mr_results_neuroticism$lower_CI),"-",sprintf("%.2f",mr_results_neuroticism$upper_CI),")")

# Format exposure name
unique(mr_results_neuroticism$exposure)
mr_results_neuroticism <- mr_results_neuroticism %>% dplyr::mutate(exposure = dplyr::case_when(exposure == "urinary_incontinence_p_value_5e-08" ~ "Urinary incontinence" ,
                                                                                               TRUE ~ exposure))
...
mr_results_neuroticism$outcome <- factor(mr_results_neuroticism$outcome, levels = c("Urinary incontinence →","Neuroticism"))

mr_results_neuroticism <- mr_results_neuroticism[order(mr_results_neuroticism$outcome),]

mr_results_neuroticism$colour <- rep(c("gray95","white"), 1)
...
p_neuroticism <- ggplot(mr_results_neuroticism, aes(x = b, y = outcome, xmin = lower_CI, xmax = upper_CI, fill = outcome)) +
  ...
  # geom_hline(aes(yintercept = order, colour = colour)) +
  geom_pointrange(shape = c(15,15), fill = "black") +
...
table_results_neuroticism$outcome <- factor(table_results_neuroticism$outcome, levels = rev(c("Urinary incontinence →","Neuroticism")))
...
png("results/forest_plots/figure_1.png", units = "mm", width=210, height=297,res = 1000)
