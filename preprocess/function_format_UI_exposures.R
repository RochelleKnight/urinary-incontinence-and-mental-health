...
#exposure_name <- "mixed_urinary_incontinence"
format_UI_exposure_data <- function(exposure_data_set,exposure_name){
  
  main <- exposure_data_set %>% filter(P < 5e-08)
  if(nrow(main)>0){
    main$phenotype <- paste0(exposure_name, "_p_value_5e-08")
    main$proxy <- "non_proxy_snp"
    main <- main %>% filter(!grepl("chr",SNP) & !is.na(SNP))
  }else{
    main <- as.data.frame(matrix(nrow = 0,ncol = 0))
  }
  ...
  if(nrow(sensitivity)>0){
    ...
    sensitivity <- sensitivity %>% filter(!grepl("chr",SNP) & !is.na(SNP))
  }else{
    sensitivity <- as.data.frame(matrix(nrow = 0,ncol = 0))
  }
  ...
  if(nrow(main)>0){
    ...
    # Needs plinkbinr package loaded
  ...
  if(nrow(sensitivity)>0){
    print("Clumping sensitivity data set")
  ...
  for(analyses in analyses_to_run){
    ...
    for(outcome_name in c("MDD","broad_depression_phenotype","neuroticism","anxiety")){
      ...
      if(outcome_name == "MDD"){
        outcome <- fread("data/raw_data/MDD.gz", fill = T)
        outcome$beta <- log(outcome$OR)
        outcome$eaf <- (outcome$FRQ_A_294322*294322 + outcome$FRQ_U_741438*741438)/(294322+741438) 
        outcome <- outcome %>% select(SNP,eaf, A1,A2,beta,SE,P) %>%
          rename(effect_allele = A1,
                 other_allele = A2,
                 se = SE)
      }else if(outcome_name == "broad_depression_phenotype"){
        ...
        outcome <- outcome %>% select(MarkerName,Freq, A1,A2,LogOR, StdErrLogOR, P) %>%
          rename(SNP = MarkerName,
                 ...
                 eaf = Freq,
                 beta = LogOR)
      }else if(outcome_name == "neuroticism"){
        outcome <- fread(file="data/raw_data/SummaryStats.txt", fill = T)
        outcome_eaf <- fread("data/raw_data/UKB_Neu_AF.txt", fill = T)
      ...
      if(length(missing_snps)>0){
        ...
        LDproxy_batch(missing_snps, pop="EUR", r2d = "r2", token = "5cb32cd8137f",append = T) #using missing snps extracts proxies and saves into txt file (combined_query_snp_list)
        ...
        proxySNPs <- proxySNPs %>% filter(query_snp != RS_Number &
                                            RS_Number != ".") 
        snp=unique(proxySNPs$query_snp)[1]
    ...
    write.csv(results, paste0("data/outcome/outcome_data_for_", exposure_name,save_name,".csv"),row.names = F)
  }
