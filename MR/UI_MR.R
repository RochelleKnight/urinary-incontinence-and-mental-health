...
#exposure_name <- "urinary_incontinence"
#outcome_name <- "MDD"
for(exposure_name in c("urinary_incontinence","mixed_urinary_incontinence","stress_urinary_incontinence","urgency_urinary_incontinence")){
  ...
  for (outcome_name in c("MDD","broad_depression_phenotype","neuroticism","anxiety")) {
    
    print(paste0("working on ",exposure_name," ",outcome_name))
    
    if(file.exists(paste0("data/exposure/exposure_", exposure_name,"_p_value_5e-08_outcome_",outcome_name, "_with_proxies.csv"))){
      ...
      exposure <- as.data.frame(matrix(nrow = 0,ncol = 0))
      outcome <- as.data.frame(matrix(nrow = 0,ncol = 0))
    }
  ...
  write.csv(compiled_mr_results,paste0("results/UI_MR/",exposure_name,"_p_value_5e-08.csv"), row.names = F)
  write.csv(compiled_heterogeneity,paste0("results/UI_MR/heterogeneity_",exposure_name,"_p_value_5e-08.csv"), row.names = F)
  write.csv(compiled_pleiotropy,paste0("results/UI_MR/pleiotropy_",exposure_name,"_p_value_5e-08.csv"), row.names = F)
  write.csv(compiled_results_single,paste0("results/UI_MR/results_single_",exposure_name,"_p_value_5e-08.csv"), row.names = F)
  write.csv(compiled_results_loo,paste0("results/UI_MR/results_loo_",exposure_name,"_p_value_5e-08.csv"), row.names = F)
  write.csv(compiled_f_stat,paste0("results/UI_MR/results_f_stat_",exposure_name,"_p_value_5e-08.csv"), row.names = F)
...
for(exposure_name in c("urinary_incontinence","mixed_urinary_incontinence","stress_urinary_incontinence","urgency_urinary_incontinence")){
  ...
  write.csv(compiled_mr_results,paste0("results/UI_MR/",exposure_name,"_p_value_5e-06.csv"), row.names = F)
  write.csv(compiled_heterogeneity,paste0("results/UI_MR/heterogeneity_",exposure_name,"_p_value_5e-06.csv"), row.names = F)
  write.csv(compiled_pleiotropy,paste0("results/UI_MR/pleiotropy_",exposure_name,"_p_value_5e-06.csv"), row.names = F)
  write.csv(compiled_results_single,paste0("results/UI_MR/results_single_",exposure_name,"_p_value_5e-06.csv"), row.names = F)
  write.csv(compiled_results_loo,paste0("results/UI_MR/results_loo_",exposure_name,"_p_value_5e-06.csv"), row.names = F)
  write.csv(compiled_f_stat,paste0("results/UI_MR/results_f_stat_",exposure_name,"_p_value_5e-06.csv"), row.names = F)
  print("Finished")
}
